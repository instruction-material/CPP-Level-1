...
int chooseAnswer() {
    ...
    std::cout << "\nGreat! Now, guess my number between " << minVal << " and "
              << maxVal << std::endl;
...
int getGuess(int answer, int guessesLimit, int guessesUsed) {
    ...
    std::cout << "You have " << guessesLimit - guessesUsed
              << " guesses remaining. What is your guess? ";
    std::cin >> guess;

    // Prints out some feedback for the user
    if (guess == answer) {
        std::cout << "Wow! Good work! That was the number! You used "
                  << guessesUsed + 1 << " guesses!" << std::endl;
...
void runGame() {
    ...
    int guessesLimit = 5;
    std::cout << "\nWelcome! In this program, you'll have " << guessesLimit
              << " guesses to guess the number I'm thinking of in the range "
                 "that you provide. Let's get started! "
              << std::endl;

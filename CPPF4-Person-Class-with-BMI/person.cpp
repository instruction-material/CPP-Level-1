...
Person::Person()
    : age(0), name("Unknown"), height(0), birthday("January 1, 1970"),
      location("Somewhere over the rainbow") {
    // Nothing to do here, we're done with initialization!
}

// our constructor using the BMI
Person::Person(int _age, std::string _name, int _height, std::string _birthday,
               std::string _location)
    : age(_age), name(_name), height(_height), birthday(_birthday),
      location(_location) {
    // Nothing to do here, we're done with initialization!
}
...
std::string Person::toString() {
    return ("This is a person named " + name + " that is currently " +
            std::to_string(age) + " years old, born on " + birthday + " at " +
            location + " and is " + stringHeight());
}
...
std::string Person::stringHeight() {
    ...
    std::string printHeight =
        std::to_string(feet) + "' " + std::to_string(inches) + "\"";
    return printHeight;
}

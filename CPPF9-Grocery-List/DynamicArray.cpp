...
DynamicArray::DynamicArray(const DynamicArray& other)
    : mySize(other.mySize), maxSize(other.maxSize), myVals(new Grocery[other.maxSize]) {
...
DynamicArray& DynamicArray::operator=(const DynamicArray& other) {
  ...
  Grocery* newVals = new Grocery[other.maxSize];
...
void DynamicArray::resize(size_t newCapacity) {
  Grocery* newVals = new Grocery[newCapacity];
  for (size_t i = 0; i < mySize; ++i) {
    newVals[i] = myVals[i];
  }
...
Grocery DynamicArray::accessVal(size_t index) const {
  if (index < mySize) {
    return myVals[index];
  } else {
